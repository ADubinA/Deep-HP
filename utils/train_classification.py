...
def main():
    ...
    parser.add_argument('--train_dataset', type=str, default=r'D:\visceral\visceral2_sliceV2\ply', help="dataset path")
    parser.add_argument('--train_dataset_labels', type=str, default=r'D:\visceral\visceral2_sliceV2\labels.csv', help="dataset path")
    parser.add_argument('--test_dataset', type=str, default=r'D:\visceral\slice_data1\ply', help="dataset path")
    parser.add_argument('--test_dataset_labels', type=str, default=r'D:\visceral\slice_data1\labels.csv', help="dataset path")
    ...
    dataset = dataloaders.VisceralData(opt.train_dataset,opt.train_dataset_labels, num_points=opt.num_points, ignore_list=[9,10,11,12])
    #Todo create real test and train datasets
    val_dataset = dataloaders.VisceralData(opt.test_dataset,opt.test_dataset_labels, num_points=opt.num_points)
    # test_dataset = dataloaders.VisceralData(opt.dataset,opt.dataset_labels, num_points=opt.num_points)
    ...
    model = LightningPointNet(PointNetCls(k=num_classes,feature_transform=True))
    tb_logger = pl_loggers.TensorBoardLogger('logs/')
    trainer = pl.Trainer(max_epochs=90, gpus=1,logger=tb_logger, log_every_n_steps=10,flush_logs_every_n_steps=100,
                         check_val_every_n_epoch=1, weights_summary='full',auto_lr_find=True,
